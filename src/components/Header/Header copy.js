...
function getTempValues(cityName, props) {
    axios.get("https://api.openweathermap.org/data/2.5/weather?q=" + cityName + "&appid=" + props.appid + "&units=metric")
    .then(res => {
        return res.data;
    })
...
function Header(props) {
    ...
    if(props.search.length > 0){
        ...
        searchRes = sliceresults.map((result, index) => {

            const response = getTempValues(result.name, props);
    ...
    return (
        <div className="hear_wrapper">
            <div className="header__container">
                ...
                <input type="text" name="search" id="search" onChange={(e) => props.searchHandler(e)} placeholder="Search" value={props.search} autoComplete="off" />
